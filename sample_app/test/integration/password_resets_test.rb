...
class PassfordFormTest < PasswordResetForm
  ...
  test "reset with wrong email" do
    get(
      edit_password_reset_path(@reset_user.reset_token, email: "")
    )
    assert_redirected_to root_url
  end
  ...
  test "reset with inactive user" do
    ...
    get(
      edit_password_reset_path(@reset_user.reset_token, email: @reset_user.email)
    )
    assert_redirected_to root_url
  end
  ...
  test "reset with right email but wrong token" do
    get(
      edit_password_reset_path('wrong token', email: @reset_user.email)
    )
    assert_redirected_to root_url
  end
...
class ExpiredTokenTest < ExpiredToken
  ...
  test "should include the word 'expired' on the page" do
    follow_redirect!
